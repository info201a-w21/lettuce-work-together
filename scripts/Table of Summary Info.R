...
max_meal_lost <-financial_df %>% 
  ...
  filter(`Meals Not Consumed due to Budget Shortfall`==max(`Meals Not Consumed due to Budget Shortfall`)) %>% 
  pull(`State Name`)

# Even though Texas as the cheapest meal cost, it has the highest meal loss due to budget shortfalls 

# Find State with least meal loss
min_meal_lost <-financial_df %>% 
  select(`State Name`,`Meals Not Consumed due to Budget Shortfall`) %>% 
  group_by(`Meals Not Consumed due to Budget Shortfall`) %>% 
  filter(`Meals Not Consumed due to Budget Shortfall`==min(`Meals Not Consumed due to Budget Shortfall`)) %>% 
  pull(`State Name`)
