...
# Join multi-year dataframes
three_yr_trend <- left_join(foodsec_18, foodsec_17, by = "State.Name") %>% 
  left_join(foodsec_16, by = "State.Name")

# Transpose the dataframe with tidyr
three_yr_trend <- gather(
  three_yr_trend,
  key = year,
  value = food_insec,
  -State.Name
...
# Plot the line graph
g <- ggplot(data = three_yr_trend) +
  geom_line(
    mapping = aes(x = year, y = food_insec, color = State.Name, group = 1)
  )