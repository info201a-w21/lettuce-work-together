...
```
## Summary
Our analysis revealed that in 2018, there was an average of `r Avg_num_food_insecure` people per state who were food insecure in the United States. By taking a closer look, we found that **`r state_max_insecurity` ** was the most insecure state of 2018, which was contrasted by `r state_min_insecurity`, who was the least food insecure. We also examined the proportion of food insecure children to food insecure adults, which was (**`r prop_food_insecure`**). This means that _for every 10 food insecure adults there were about 3 food insecure children in the U.S. in 2018_.

  Given that food prices typically impact access to food, we also compared meal costs across the United States. The **average meal cost was `r Avg_meal_cost`**; however, **`r cheapest_meal_2018` had the lowest meal cost of `r price_cheapest_meal`**, while **`r most_expensive_meal` had the highest meal cost of `r price_most_expensive_meal`**. Using the 2018 food budget shortfall data, we were able to calculate that an average of `r num_meals_not_consumed` meals were _not_ consumed in the U.S. due to the shortfalls. In other words, each food insecure adult experienced an estimated of **`r num_days_lost` days-worth of missed meals in 2018**.
