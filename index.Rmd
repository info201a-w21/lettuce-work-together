...
I have included this chart in order to reveal the national trend of food
insecurity rate in the U.S., between 2009 and 2018. In the original data set,
food insecurity rate for each state is included. I have aggregated these state
level food insecurity rates by each year and obtained its average, which is the
national average of food insecurity rate each year. In the chart we can see that
the rate sees an increase from 2009 to 2011, then sees a decline until 2018. 
One possible explanation for this may be the economic impact caused by the 2008
financial crisis that lasted through early 2010s, and the gradual economic 
recovery afterwards. 
...
- With a **range** in data of about **$200/person****, most states have roughly the same level of need.
...
plot(chart)
```
This scatterplot observes the correlation between food insecurity and meal cost in 2018. Each point represents a US state; some states have similar rates and there are few outliers. Using a scatterplot helps interpret and visualize the data easily. From the chart, we can tell that food insecurity and meal cost differ between states. One outlier in the data is Washington DC, which has the highest meal cost, but an average insecurity rate. On the flip side, Mississippi has a relatively low meal cost but the highest food insecurity rate. These observations raise questions about the impact of meal cost on food insecurity and vice versa. 